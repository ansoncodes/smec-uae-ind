...
The prototype's primitives on an industrial palette. Token names are
unchanged (`--copper`, `--steel`, `--grad-hot`, `--grad-cool` …) so every
module resolves; only the values moved:
...
- **Sustainability artwork** — the one green moment on a gunmetal page, so its
...
holds the copy at readable contrast and tints the photograph toward the
charcoal ground so it joins the palette. The scrims are weighted **horizontally** on desktop, where
